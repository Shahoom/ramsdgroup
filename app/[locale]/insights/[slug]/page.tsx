...
export default async function ArticlePage({ params }: { params: Promise<{ locale: Locale; slug: string }> }) { const { locale, slug } = await params; const entry = await getEntryBySlug("article", locale, slug); if (!entry) notFound(); const related = await listRelatedEntries(entry, 3); return <><JsonLd data={entryDetailJsonLd(locale, entry)} /><EntryDetail entry={entry} related={related} /></>; }
